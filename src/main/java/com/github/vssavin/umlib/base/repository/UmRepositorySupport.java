...
/**
 * Executes the specified function and wraps any exception using the specified exception class.
 *
...
public class UmRepositorySupport<T, R> {
    ...
    private RuntimeException wrapException(Exception e, Class<? extends RuntimeException> exceptionClass, String message) {
        Constructor<?>[] ctors = exceptionClass.getDeclaredConstructors();
        Constructor<?> ctor = null;
        RuntimeException wrappedException =
                new UmRepositorySupportException("An error occurred while wrapping the exception!");
