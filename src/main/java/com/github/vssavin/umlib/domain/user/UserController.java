...
import com.github.vssavin.umlib.domain.email.EmailService;
...
final class UserController extends UmControllerBase {
	...
	ModelAndView confirmUser(final HttpServletRequest request, @RequestParam final String login,
		...
		String resultMessage = LocaleConfig.getMessage(PAGE_CONFIRM_USER, MessageKey.CONFIRM_SUCCESS_MESSAGE.getKey(),
				lang);
		...
		try {
			...
			resultMessage = LocaleConfig.getMessage(PAGE_CONFIRM_USER, MessageKey.CONFIRM_FAILED_MESSAGE.getKey(),
					lang);
		}
	...
	ModelAndView performPasswordRecovery(final HttpServletRequest request, @RequestParam final String loginOrEmail,
		...
		try {
			...
			if (optionalRecoveryId.isPresent()) {
				...
				String message = umConfig.getApplicationUrl() + USER_CONTROLLER_PATH + "/" + PAGE_RECOVERY_PASSWORD
						+ "?recoveryId=" + optionalRecoveryId.get();
	...
	ModelAndView performUserEdit(final HttpServletRequest request, final HttpServletResponse response,
		...
		try {
			...
			String successMsg = LocaleConfig.getMessage("userEdit", MessageKey.USER_EDIT_SUCCESS_MESSAGE.getKey(),
					lang);
			modelAndView.addObject(SUCCESS_MSG_ATTRIBUTE, successMsg);
		}
		catch (Exception e) {
		...
		String successMsg = LocaleConfig.getMessage(PAGE_USER_EDIT, MessageKey.USER_EDIT_SUCCESS_MESSAGE.getKey(),
				lang);
		modelAndView.addObject(SUCCESS_ATTRIBUTE, true);
		modelAndView.addObject(SUCCESS_MSG_ATTRIBUTE, successMsg);
		return modelAndView;
